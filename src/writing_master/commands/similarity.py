#!/usr/bin/env python3
"""字符 n-gram Jaccard 相似度检测 - 用于跨平台改写的去重检查。

改编自 wewrite.commands.similarity_check，适配 AI Writing Master。

用法：
    writing-master similarity a.md b.md [c.md ...]   # 两两最大相似度
    writing-master similarity a.md b.md --json       # JSON输出供 agent 解析
"""
...
def similarity(a: str, b: str, n: int = 3) -> float:
    """计算字符 n-gram 的 Jaccard 相似度，返回 0-1。"""
...
def main(argv=None) -> int:
    """主函数入口。"""
    ap = argparse.ArgumentParser(description="字符 n-gram Jaccard 相似度检测")
    ap.add_argument("files", nargs="+", help="待比较的文本文件（≥2个）")
    ...
    if args.json:
        ...
        print(f"最大相似度: {m:.4f}")
        print(f"{'✅ 通过' if m <= 0.6 else '❌ 不通过'} (阈值: 0.6)")
