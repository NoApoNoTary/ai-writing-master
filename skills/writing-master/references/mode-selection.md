...
若所选模式未就绪，系统不会自动替你切换；你可以选择改用实际可用的模式（系统会先说明损失）、保留任务，或取消。
```
...
- `unavailable`：暂停当前所选模式；素材提取、调研、正文生成、视觉生成和角色派发的调用次数必须为 0。不自动切换到 quick、standard 或其他模式，也不创建模拟结果。

`unavailable` 时向用户显示下面的固定结果和选择：
...
发送时把 `{模式显示名}` 替换为“快速草稿”“标准写作”或“深度写作”，并用当前安装版本替换 `VERSION`；无法确定时写 `unknown`。可用模式及其损失说明必须使用当前 capability preflight 的实际结果；用户选择改用模式后，在运行目录记录原模式、用户选择、变更原因和预期损失，并重新执行所选模式的就绪检查。普通错误正文只包含用户结果、诊断编号和版本。Handoff Runtime、宿主能力、异常类型和内部异常栈只写入诊断详情。只提醒用户提交 Issue，不自动创建 Issue，也不生成 Issue 草稿。
