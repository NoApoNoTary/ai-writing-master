...
`status.json` 记录 `persona_mode`、`persona_snapshot: none | pending | ready | unavailable`、来源路径、可见版本、原始 Skill SHA-256 和 Persona Brief SHA-256。恢复任务只校验并复用任务内 `persona-skill.md` 与 `persona-brief.md`，不回读或从任何内置、外部来源重建当前版本。内容契约确认后，若 `persona_snapshot=ready` 的 Persona、模式、背景选项或项目背景发生变化，先展示差异并创建新 Writing run；新 run 的 `source_task_id` 记录原 `task_id`，`source_change_kind: persona`，`source_change_sha256` 记录变更摘要 hash，`contract` 从 `pending` 重新确认。原 run 的关联字段、`persona-skill.md`、`persona-brief.md` 和 hash 保持不变。
...
旧任务缺少 Persona 字段时按 `none` 处理。Persona 文件缺失或 hash 不一致时停止依赖 Persona 的策划、写作和审校，不从任何内置或外部来源重建一个看似相同的版本。
