...
5. `writing-master quality` 提供套话、句长、段落、副词和词汇等机械预警，以及独立的高置信过程泄漏候选；它不验证事实、论证、原创性或作者声音。
...
#### 5. Prompt/process leakage 与标题

- 检查标题以及每个 H1/H2/H3；每个标题都要回答读者问题、组织读者理解或提示读者行动。
- 若标题或正文服务作者/编辑的取舍，而不是读者问题，至少标记为 `major`；高置信命中在人工复核前保持 `blocking`。
- “是否应该写某内容、广告判断、发布/生图/来源策略、用户要求、内部产物名”等只影响编辑决策，除非文章主题本身讨论编辑流程，否则不得写入读者正文。
- 过程泄漏 issue 必须保留标题或正文的原句、行号/章节位置、规则 ID 和修订边界；不得只写“有内部感”。
...
逐项查看命中位置，再判断是否修改。`findings` 含标题和正文的行号、原句与规则 ID，不计入机械分数；高置信候选在人工复核前按 blocking 处理。机械分数较低不自动判定文章失败，分数较高也不代表文章已经通过编辑审查。
