...
价值是多维度的，包括但不限于：
- **信息价值**：快速了解热点、技术趋势、产品动态
- **决策价值**：帮助判断"这个值得关注吗？""这个趋势要跟吗？"
- **情绪价值**：共鸣、启发、"原来不只我这么想"
- **社交价值**：转发到朋友圈/群聊时，展示自己的洞察力
- **时间价值**：15-20 分钟得到高质量成品，而不是几小时
...
**不是什么**：
- ❌ 不是证据审查系统（证据追溯是内部质量保障机制）
- ❌ 不是严肃报道工具（目标场景是技术博客、产品体验、行业观察）
- ❌ 不是合规系统（用创作流程，不是审批流程）

更多产品设计原则见 [PRODUCT_VISION.md](docs/PRODUCT_VISION.md) 和 [DESIGN_PRINCIPLES.md](skills/writing-master/DESIGN_PRINCIPLES.md)。
...
**产品设计文档**：
- [产品愿景](docs/PRODUCT_VISION.md) — 为什么做这个系统，核心原则是什么
- [设计原则](skills/writing-master/DESIGN_PRINCIPLES.md) — 实现功能时的指导原则
- [Agent 指引](skills/writing-master/AGENT.md) — AI Agent 工作时的边界和规则
...
**最近更新（2026-08-07）**：
- ✅ 产品转型：从"证据审查系统"转变为"价值导向写作助手"
- ✅ 三级证据等级：relaxed/balanced/strict，按场景选择
- ✅ 快速模式简化：4-5 分钟，只生成必要文件
- ✅ 自动调研：AI 热点自动搜索，无需手动提供素材
- ✅ 内容契约简化：减少确认点，只问阻断性问题
- ✅ 标准模式优化：balanced 只验证核心数据，观点不需要 claim_id
...
| 模式 | 执行方式 | 证据等级 | 耗时 | 适合场景 |
|---|---|---|---|---|
| 快速草稿 | 当前 Agent | relaxed | 4-5 分钟 | 先获得一个有依据、可继续讨论的版本 |
| 标准写作 | 当前 Agent | balanced | 15-20 分钟 | 常规长文和可发布文章；默认推荐 |
| 深度写作 | Lead + 专项角色 | strict | 1 小时+ | 重要长文，需要隔离研究、写作和审计上下文 |

**证据等级说明**：
- **relaxed**（快速模式）：只检查明显错误，不要求来源；只生成 4-5 个必要文件
- **balanced**（标准模式，默认）：只验证核心数据（价格、日期、版本号、性能指标），作者观点、常识性陈述、个人经历不需要 claim_id；自动调研 AI 热点
