...
| 深度写作 | Lead + 专项子代理 | 重要长文，需要隔离研究、写作和审计上下文 |

**多 Agent 不是默认入口。** 对新建完整文章，只有用户选择深度写作时才创建子代理。用户已经在当前请求中说出“快速”“标准”“深度”或“多 Agent 写作”等明确模式时，直接采用，不重复询问，也不根据题目难度隐式代选。

洗稿、平台改写和只做标题、审校、选题等单一模块请求不触发这道完整文章模式闸门。`writing-rewrite` 默认也是单 Agent，只有用户明确要求深度改写或多 Agent 时才按平台隔离代理。
...
深度写作沿用同一条链路，但把高干扰环节拆给 fresh-context 角色：
...
代理之间通过运行目录中的文件产物通信，不把父对话全文复制给所有角色。
...
写作任务会在 `runs/{task_id}/` 中保存 `status.json`、Brief、来源、主张、素材清单、草稿和审校报告。具体文件随所选模式和任务需求变化。
